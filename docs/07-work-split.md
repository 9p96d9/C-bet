...
## 現状スナップショット（2026-07-06 / 122ページ・警告ゼロ・リンク切れゼロ）

S1〜S8の骨格とシードは**実装済み**。目的別コース（/goals/）とビルド拡張も完了。以降は主に「量」の増築フェーズ。
※ `main` と作業ブランチ `claude/infant-development-resource-8ovg6w` は現在**同一内容**（最新）。

| セクション | 現状 | 次の目標 |
|---|---|---|
| 科学 `01-science` | **14章** | +（腸脳相関・実行機能の育て方を独立章に・きょうだい/出生順・睡眠と昼寝の設計 等） |
...
| 図鑑 `05-activities` | **63枚**（literacy 9・music 10・play 12・fine-motor 14・math 15・gross 16・sensory 16・EF 25・social 26・language 34） | **各ドメイン15枚／総150枚**目標。薄い所＝literacy, music, play, fine-motor |
| 目的別コース `/goals/` | 4コース実装済 | 活動を足せば自動で反映（`build.mjs` の GOALS 定義で科学リンクを調整可） |
| 環境 `06-environment` | 3章 | +（スクリーン方針の独立章・きょうだい環境・安全と危険のバランス・モンテッソーリ的家庭の設え） |
| 神話 `07-myths` | 17枚 | +（早期英才教育・IQテスト・砂糖と多動・胎教・スマホで泣き止ませ 等） |
