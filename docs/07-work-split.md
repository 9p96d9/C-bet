...
## 現状スナップショット（2026-08-15 / 285ページ・警告ゼロ・リンク切れゼロ）

S1〜S8の骨格とシードは**実装済み**。目的別コース（/goals/）・ビルド拡張・**無料デプロイ（GitHub Pages / gh-pages方式）** も完了。フェーズ2の「量」の増築を継続中。
※ 公開URL: https://9p96d9.github.io/sodate-no-dodai/ 。`main` へ push すると自動ビルド・デプロイ（→ `docs/08-deploy.md`）。
※ ビルドは**内部リンク切れ・アンカー切れを自動検査**する（`✓ 内部リンク検査: N本 / Mページ`）。警告ゼロを維持すること。

| セクション | 現状 | 次の目標 |
|---|---|---|
| 科学 `01-science` | **24章**（父母以外の養育者・生活リズムを追加） | +（随時。既存24章と重ならない切り口を選ぶ） |
| 父親 `02-fathers-edge` | **7章**（父自身のコンディションを追加） | +（単身赴任・祖父母との連携 等、随時） |
| ドメイン `03-domains` | 全10ハブ＋**サブ記事30本（全10ドメイン×3本）** | 4本目以降は各論をさらに深める（`content/03-domains/<domain>/<slug>.md` を置くだけ。→ schema 02 の `domain-article`）。ハブの重複にならない切り口を選ぶこと |
| 月齢 `04-roadmap` | **80/80マス完成** | 完了。必要なら各マスの記述を深める程度 |
| 図鑑 `05-activities` | **174枚**（literacy・music を底上げ） | 最少ドメインは music 35・literacy 36。以降は新章に対応するカードを随時追加（`act-<テーマ>.mjs` を新規ファイルで） |
| 目的別コース `/goals/` | 4コース実装済 | 活動を足せば自動で反映（`build.mjs` の GOALS 定義で科学リンクを調整可） |
| 環境 `06-environment` | **6章**（外出と移動の設計を追加） | +（きょうだい環境・季節/天候の設計 等、随時） |
| 神話 `07-myths` | **36枚**（保育園と愛着・右脳教育・父は母の代用を追加） | +（随時） |
| 付録 `09-appendix` | 用語71/文献43 | 各コンテンツ増加に合わせ随時追記 |
| はじめに `00-intro` | **4章**（最初の1週間・続け方を追加） | 入口は充足。以降は必要に応じて |
| 記録 `08-track` | **5章**（健診の活用・相談先の地図・気になったときの動き方を追加） | +（随時。制度の記述は「自治体で確認」の注記を必ず添える） |
| デプロイ/SEO | **完了**（自動デプロイ・OGP・sitemap.xml・robots.txt・404） | 独自ドメイン・OG画像を足すなら任意 |
| 検索 `/search/` | **実装済**（依存ゼロのクライアントサイド検索・索引390件） | コンテンツ追加時は索引が自動更新される（`buildTitleIndex` が収集） |
| 相互リンク | **自動化済**: ドメインハブ→該当活動（例: language 63枚）、月齢帯→該当活動、カードの `related` 描画 | 活動を足せば各所に自動で載る。手動リストは作らない |
