...
## 現状スナップショット（2026-08-14 / 268ページ・警告ゼロ・リンク切れゼロ）
...
| 図鑑 `05-activities` | **162枚**（切り替え・待ち時間・外出・ひとり遊びを追加） | 全ドメイン29枚以上。以降は新章に対応するカードを随時追加（`act-<テーマ>.mjs` を新規ファイルで） |
...
| 検索 `/search/` | **実装済**（依存ゼロのクライアントサイド検索・索引358件） | コンテンツ追加時は索引が自動更新される（`buildTitleIndex` が収集） |
